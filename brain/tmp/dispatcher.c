 ...
 * Input: struct packet_data_t pointer
 * Return: 0 for no error, else error code
 */
...
/* 
 * Function used to parse the incoming packet.
 * Input: pkt: Input packet
 * pdata: Pointer to output packet, that holds all data
 * Return: 0 for no error, else error code
 */
...
/*
 * Main dispatcher function. This function is the entry point
 * to the whole dispacter functionality. User's will call the
 * dispacted() with the pointer to the memory holding the packet
 * data. 
 * Input: pkt: Pointer to input data. (can be motor/display data)
 * Return: 0 for no error, else error code
 */
